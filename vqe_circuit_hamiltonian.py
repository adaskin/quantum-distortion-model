...
class VQEHamiltonian:
    ...
    @staticmethod
    def get_eigenvalues(
        hamiltonian: qml.Hamiltonian, n_qubits: int, method: str = "matrix"
    ) -> np.ndarray:
...
class VQECircuit:
    ...
    def _circuit_template(
        ...
        for layer in range(self.layers):
            ...
            for q in range(self.n_qubits):
                ...
                if noisy_circuit:
                    self._apply_noise(
                        noise_level=noise_level, wires=q, noise_type=noise_type
                    )
    ...
    def _apply_noise(
        self, noise_level: float = 0.1, wires: int = 0, noise_type="combined"
    ):
