...
class VQEPlotter:
    ...
    def __init__(
        self, noise_schedule: NoiseSchedule, max_iterations: int, n_qubits: int
    ):
        self.noise_schedule = noise_schedule
        self.max_iterations = max_iterations
        self.n_qubits = n_qubits

    def plot_four_case_results(
        self, histories: Dict[str, VQEResults], ground_energy: float
    ) -> None:
        ...
        fig.suptitle(
            f"VQE Comparison: Distortion Framework for {self.n_qubits} qubits",
            fontweight="bold",
            fontsize=24,
        )
    ...
    def _plot_four_case_energy(
        self,
        ax,
        histories: Dict[str, VQEResults],
        ...
        for idx, (start, end, noise_level) in enumerate(
            self.noise_schedule.noise_intervals
        ):
            if noise_level > 0:
    ...
    def _plot_best_energy_comparison(
        self,
        ax,
        histories: Dict[str, VQEResults],
        ground_energy: Optional[float] = None,
    ...
    def _plot_performance_gaps(self, ax, histories: Dict[str, VQEResults]) -> None:
        ...
        gaps = {
            "Ideal+Recovery": [
                ir - i
                for ir, i in zip(histories["ideal_recovery"].energies, ideal_energies)
            ],
            "Noisy": [
                n - i for n, i in zip(histories["noisy"].energies, ideal_energies)
            ],
            "Noisy+Recovery": [
                nr - i
                for nr, i in zip(histories["noisy_recovery"].energies, ideal_energies)
            ],
        }
        ...
        ax.set_title("Gaps Relative to Ideal VQE")
        ax.legend()
        ax.grid(True, alpha=0.3)
    ...
    def _plot_distortion_timelines(self, ax, histories: Dict[str, VQEResults]) -> None:
        ...
        if histories["ideal_recovery"].distortions and len(
            histories["ideal_recovery"].distortions
        ) == len(iterations):
            ax.plot(
                iterations,
                histories["ideal_recovery"].distortions,
                "c--",
                label="Ideal+Recovery Distortion",
                linewidth=2,
                alpha=0.8,
            )

        # Plot noisy+recovery distortions and thresholds
        if histories["noisy_recovery"].distortions and len(
            histories["noisy_recovery"].distortions
        ) == len(iterations):
            ax.plot(
                iterations,
                histories["noisy_recovery"].distortions,
                "g-",
                label="Noisy+Recovery Distortion",
                linewidth=2.5,
                alpha=0.9,
            )

            if histories["noisy_recovery"].thresholds and len(
                histories["noisy_recovery"].thresholds
            ) == len(iterations):
                ax.plot(
                    iterations,
                    histories["noisy_recovery"].thresholds,
        ...
        ax.set_title("Distortion in Recovery Cases")
        ax.legend()
        ax.grid(True, alpha=0.3)
    ...
    def _plot_recovery_analysis(self, ax, histories: Dict[str, VQEResults]) -> None:
        ...
        ax.scatter(
            all_events, y_positions, c=colors, s=100, alpha=0.8, edgecolors="black"
        )
        ...
        ax.set_ylim([0, 1])
        ax.set_title(f"Recovery Events-Total: {len(all_events)}")
        ax.grid(True, alpha=0.3)
    ...
    def _plot_noise_schedule(self, ax) -> None:
        ...
        ax.plot(
            iterations, noise_levels, "b-", linewidth=2, label="Noise Level", alpha=0.8
        )
        ...
        for start, end, noise_level in self.noise_schedule.noise_intervals:
            if noise_level > 0:
                ax.axvspan(
                    ...
                    label=(
                        f"Noise Interval"
                        if start == self.noise_schedule.noise_intervals[0][0]
                        else ""
                    ),
                )
