...
class ConvergenceRateDistortion:
    ...
    def __init__(self, delta_min=1e-8, epsilon=1e-8):
...
class CompositeDistortionModel:
    ...
    def __init__(
        ...
        # Normalize weights
        total_weight = (
            weight_energy + weight_fidelity + weight_parameter + weight_convergence
        )
        if total_weight > 0:
    ...
    def compute_component_distortions(
        self, energy_history, fidelity_history, params_current, params_previous
    ):
        ...
        # Get energy context
        energy_current = (
            energy_history[-1] if energy_history and len(energy_history) > 0 else None
        )
        energy_previous = (
            energy_history[-2] if energy_history and len(energy_history) > 1 else None
        )
