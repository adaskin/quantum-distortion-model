...
class RecoveryStrategy:
    ...
    def compute_recovery(
        ...
        if (not energies or not params_history or 
            len(energies) < self.min_history or 
            len(params_history) < self.min_history):
            return (
        ...
        if (self.use_gradient and 
            recovery_type in ["very_mild_adjust","mild_adjust"] and 
            normalized_distortion > 0.0):
        ...
        if recovered is not None and current_params is not None:
            recovered = self._bound_change(recovered, current_params, self.max_param_change)
        
        applied = recovery_type != "no_recovery"
        return recovered, new_lr, applied, recovery_type

    def _best_recent(self, energies: List[float], 
                     params_history: List[np.ndarray]) -> Tuple[Optional[np.ndarray], Optional[float]]:
        """Find best parameters from recent history."""
        if not energies or not params_history:
            return None, None
            
        lookback = min(self.recent_lookback, 
                       len(energies), 
                       len(params_history))
        if lookback == 0:
            return None, None
    ...
    def _smoothed_params(self, params_history: List[np.ndarray], window: int = 10) -> Optional[np.ndarray]:
    ...
    def _gradient_recovery(self, energies: List[float], params_history: List[np.ndarray]) -> Optional[np.ndarray]:
    ...
    def _bound_change(self, new_params: np.ndarray, old_params: np.ndarray, 
                      max_change: float) -> np.ndarray:
...
class RecoveryManager:
    ...
    def __init__(
        ...
        self.recovery_types = []
        self.effectiveness = []  # List of tuples (energy_before, energy_after, improved)
    ...
    def update_effectiveness(self, energy_before: float, energy_after: float) -> None:
        ...
        improved = energy_after < old_energy if (old_energy is not None and energy_after is not None) else None
    ...
    def _current_cooldown(self) -> int:
        ...
        ineffective_count = sum(1 for _, _, improved in recent_effectiveness if improved is False)
    ...
    def get_stats(self) -> dict:
        ...
        effective_count = sum(1 for _, _, improved in self.effectiveness if improved is True)
        ineffective_count = sum(1 for _, _, improved in self.effectiveness if improved is False)
        unknown_count = sum(1 for _, _, improved in self.effectiveness if improved is None)
        ...
        return {
            ...
            "cooldown_remaining": max(0, self._current_cooldown() - (self.last_iteration + self.cooldown)),
        }